...
export default function EditBusiness() {
  ...
  const form = useForm({
    initialValues: { businessName:'', businessOwnerName:'', businessType:'', phoneNumber:'', address:'', email: rowData.name, businessDescription:'' },

    // functions will be used to validate values at corresponding key
    validate: {
      businessName: (value) => (/^[A-Za-z ]{3,30}$/.test(value) ? null : 'Business Name Should Contain Atleast 3 Alphabets'),
      businessOwnerName: (value) => (/^[A-Za-z ]{3,30}$/.test(value) ? null : 'Please Select Business Owner Name'),
      businessType: (value) => (/^[a-zA-Z]{3,20}$/.test(value) ? null : 'Please Select Business Type'),
  ...
  return (
    <Paper withBorder shadow="md" p={35}  radius="md">
      ...
      <form onSubmit={form.onSubmit(console.log)} >
        <Box>
        <Select withAsterisk size='md' label="Business Type" placeholder="Select Business Type" {...form.getInputProps('businessType')}
        data={[
      ...
      <Box mt="md" className={classes.responsiveContainer}>
        ...
        <Box mt="md" >
        <Textarea withAsterisk size='md' label="Business Description" placeholder="Enter Business Description: Car Business." {...form.getInputProps('businessDescription')}  />
        </Box>
      <Box mt="md" >
      <FileInput
